...
from Bert import Dataset, BertClassifier_npre, label_dict_shl, label_dict_gl
...
def training(model, train_data, val_data, learning_rate, epochs):
    ...
    fig.tight_layout()  
    plt.savefig('train_val_loss_acc_npre.png')

def evaluate(model, test_data):
    model.eval()
    test = Dataset(test_data)
    total_acc_test = 0
    #judge whether cuda is available
    use_cuda = torch.cuda.is_available()
    device = torch.device('cuda' if use_cuda else 'cpu')
